   ...
   const handleImageSubmit = (e) =>{
        if(files.length > 0 && files.length + formData.imageUrls.length < 7 ){
           ...
           setImageUploadError(false)
            ...
            const promises = []//more than one img so more than one async behaviour need to wait for them
            for (let i =0; i< files.length; i++){
                promises.push(storeImage(files[i]))//all the download url are going to be stored in this promise
            }
            Promise.all(promises).then((urls)=>{
                ...
                setImageUploadError('Image upload error failed (2mb max per image')
    ...
    <main className='p-3 max-w-4xl mx-auto'>
        ...
        <form className='flex flex-col sm:flex-row gap-4' >
            ...
            <div className='flex flex-col flex-1 gap-4'>
                ...
                <span className='font-normal text-gra-700 ml-2'>The first image will be the cover (max 6)</span>
                </p>
                <div className='flex gap-4' >
                    ...
                    <button disabled={uploading} type='button' onClick={handleImageSubmit} className='p-3 text-green-700 border border-green-700 rounded uppercase hover:shadow-lg disabled:opacity-80'>
                       {uploading? 'Uploading...':'Uplaod'}</button>
